var ArabicNumerals = ArabicNumerals || {};

ArabicNumerals.createNameSpace = function (namespace) {

    //get the namespace string and split it
    var namespaceArr = namespace.split('.');
    var parent = ArabicNumerals;

    // excluding the parent
    if (namespaceArr[0] === 'ArabicNumerals') {
...
ArabicNumerals.createNameSpace("ArabicNumerals.Cookies");

ArabicNumerals.Cookies.getCookie = function (ck_name) {
...
ArabicNumerals.Cookies.setCookie = function (ck_name, value, exdays) {
    var exdate = new Date();
